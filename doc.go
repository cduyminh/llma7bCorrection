...
func ReadDocxFile(path string) (*ReplaceDocx, error) {
	...
	// Read and return the ReplaceDocx instance
	return ReadDocx(zipData)
}
...
func ReadDocx(reader ZipData) (*ReplaceDocx, error) {
	...
	// Retrieve and store raw content
	rawContent, err := readRawContent(reader.files())
	if err != nil {
		return nil, err
	}
...
func readRawContent(files []*zip.File) (string, error) {
	...
	rawContent, err := rawDocToString(documentReader)
...
func printFileStats(fileInfo os.FileInfo) {
	// Print file statistics
...
func (d *Docx) RetrieveRawContent() (string, error) {
	documentFile, err := retrieveWordDoc(d.files)
	if err != nil {
		return "", err
	}
	documentReader, err := documentFile.Open()
...
func rawDocToString(reader io.Reader) (string, error) {
	b, err := ioutil.ReadAll(reader)
	if err != nil {
		return "", err
	}

	// Remove content between <w:fldData xml:space="preserve"> and </w:fldData>
	reFldData := regexp.MustCompile(`<w:fldData xml:space="preserve">.*?</w:fldData>`)
	content := reFldData.ReplaceAllString(string(b), " ")

	// Remove content between <w:instrText xml:space="preserve">and </w:instrText>
	reInstrData := regexp.MustCompile(`<w:instrText xml:space="preserve">.*?</w:instrText>`)
	content = reInstrData.ReplaceAllString(content, " ")

	// Replace Paragraph Tag with new line
	pTags := regexp.MustCompile(`</w:p>`)
	content = pTags.ReplaceAllString(content, "\n")

	// Remove everything between < and >
	reTags := regexp.MustCompile(`<.*?>`)
	content = reTags.ReplaceAllString(content, "")

	// Remove any string longer than 20 characters
	reLongStrings := regexp.MustCompile(`\S{30,}`)
	content = reLongStrings.ReplaceAllString(content, "")

	// Replace consecutive newlines with one newline
	reConsecutiveNewlines := regexp.MustCompile(`\s*\n+\s*`)
	content = reConsecutiveNewlines.ReplaceAllString(content, "\n")
